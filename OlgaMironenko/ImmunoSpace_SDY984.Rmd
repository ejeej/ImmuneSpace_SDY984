...
Всего получается `r length(genes_int_both5_init)` генов в модели с исходными значениями для экспрессии и `r length(genes_int_both5_rank)` гена в модели с рангами для экспрессии, для которых на 5%-ном уровне значимости статистически значим и основной эффект для ответа, и эффект пересечения ответа со временем, т.е. испытуемые с разным уровнем ответа значимо отличались не только по среднему значению экспрессии данных генов на момент baseline, но и по его динамике. При этом эти списки пересекаются по `r length(intersect(genes_int_both5_init, genes_int_both5_rank))` генам: `r paste(intersect(genes_int_both5_init, genes_int_both5_rank), collapse = ", ")`.

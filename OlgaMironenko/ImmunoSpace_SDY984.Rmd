...
Всего получается `r length(genes_sig$init_int_b2b3)` генов в модели с исходными значениями для экспрессии и `r length(genes_sig$rank_int_b2b3)` гена в модели с рангами для экспрессии, для которых на 5%-ном уровне значимости статистически значим и основной эффект для ответа, и эффект пересечения ответа со временем, т.е. испытуемые с разным уровнем ответа значимо отличались не только по среднему значению экспрессии данных генов на момент baseline, но и по его динамике. При этом эти списки пересекаются по `r length(intersect(genes_sig$init_int_b2b3, genes_sig$rank_int_b2b3))` генам: `r paste(intersect(genes_sig$init_int_b2b3, genes_sig$rank_int_b2b3), collapse = ", ")`.

Ещё для `r length(genes_sig$init_int_b2)` (`r length(genes_sig$rank_int_b2)`) генов в соответствующих моделях был значим только основной эффект, т.е. испытуемые с разным ответом на вакцинацию изначально различались по экспрессии этих генов, но её динамика была схожей в этих группах.

Ещё для `r length(genes_sig$init_int_b3)` (`r length(genes_sig$rank_int_b3)`) генов в соответствующих моделях был значим только эффект пересечения, т.е. испытуемые с разным ответом на вакцинацию изначально не различались по экспрессии этих генов, но её динамика была в этих группах разной.
...
p1 <- ggplot(lmer_v_int_plt %>% filter(model == "Expression as is"), aes(x = dydx, y = p.value)) +
  geom_point(aes(color = sig), alpha = 0.5, show.legend = FALSE) +
  ...
  geom_text_repel(aes(label = gene), lmer_v_int_plt %>% filter(model == "Expression as is" & sig), 
  ...
  labs(x = "ATE for response at the time point", y = bquote(-log[10](p[ATE])), 
       title = "Figure 5(1). P-values vs. effect sizes for response (Expression as is)",
       subtitle = "Average marginal effects after linear mixed models with the interaction term") +
  theme_classic(base_size = 12) +
  theme(legend.position = "bottom",
        panel.grid.major = element_line(linewidth = .2, color = '#ebebebFF'),
...
p2 <- ggplot(lmer_v_int_plt %>% filter(model != "Expression as is"), aes(x = dydx, y = p.value)) +
  geom_point(aes(color = sig), alpha = 0.5, show.legend = FALSE) +
  geom_hline(aes(yintercept = 1), linewidth = 0.3, color = "#E15759", linetype = "dashed") +
  geom_hline(aes(yintercept = critv), linewidth = 0.3, color = "red") +
  geom_text_repel(aes(label = gene), lmer_v_int_plt %>% filter(model != "Expression as is" & sig), 
  ...
  facet_rep_grid(~ time, repeat.tick.labels = TRUE, scales = "free", switch = "y") +
  labs(x = "ATE for response at the time point", y = bquote(-log[10](p[ATE])), 
       title = "Figure 5(2). P-values vs. effect sizes for response (Rank by expression)",
       subtitle = "Average marginal effects after linear mixed models with the interaction term") +
  theme_classic(base_size = 12) +
  theme(legend.position = "bottom",
        panel.grid.major = element_line(linewidth = .2, color = '#ebebebFF'),

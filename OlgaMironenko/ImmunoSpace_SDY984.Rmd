...
```{r fig4_upset_int_noint, fig.width=10, fig.height=12}
upset_plt_df <- cbind(
...
Объедними результаты по моделям с исходными данными по экспрессии и с рангами генов по ней - получится, что для `r nrow(genes_sig_ame7)` генов ATE в 7 дней был значим в одной из этих моделей, но не значим ни в одной из них в 0-3 дня. Для `r nrow(genes_sig_ame03)` генов ATE был значим одновременно в 0-1-3 дня в любой из этих моделей и незначим в 7 дней ни в одной из них. Для каждого из этих наборов генов отберём наиболее представленные **биологические процессы**, в которых участвуют их продукты, и покажем их на графике ниже.
...
```{r fig8_volcano, fig.width=6, fig.height=5}
...
```{r fig9_volcano_time, fig.width=6, fig.height=12}
...
```{r fig9_pres, echo=FALSE, eval=FALSE}
# Для презентации
...
p1 <- ggplot(lmer_v_int_plt %>% filter(expr == "Expression as is"), aes(x = estimate, y = logp)) +
  ...
  labs(x = "ATE for response at the time point", y = bquote(-log[10](p[ATE])), 
       title = "Figure 9(1). P-values vs. effect sizes for response (Expression as is)",
       subtitle = "Average marginal effects after linear mixed models with the interaction term") +
  theme_classic(base_size = 12) +
  theme(legend.position = "bottom",
        panel.grid.major = element_line(linewidth = .2, color = '#ebebebFF'),
...
p2 <- ggplot(lmer_v_int_plt %>% filter(expr != "Expression as is"), aes(x = estimate, y = logp)) +
  ...
  facet_rep_grid(~ time, repeat.tick.labels = TRUE, scales = "free", switch = "y") +
  labs(x = "ATE for response at the time point", y = bquote(-log[10](p[ATE])), 
       title = "Figure 9(2). P-values vs. effect sizes for response (Rank by expression)",
       subtitle = "Average marginal effects after linear mixed models with the interaction term") +
  theme_classic(base_size = 12) +
  theme(legend.position = "bottom",
        panel.grid.major = element_line(linewidth = .2, color = '#ebebebFF'),
...
```{r fig10_bps_big, fig.width=9, fig.height=7}
gobp_time <- bind_rows(
...
```{r fig11_bps_big_imm, fig.width=9, fig.height=10}
...
```{r fig12_upset_logreg, fig.width=10, fig.height=5}
# Whether each gene was significant in any model
genes_maxvar <- cbind(
  genes_maxvar,
...
```{r fig13_compmodels, fig.width=7, fig.height=8}
upset_plt_df_1 <- genes_maxvar %>%
...
```{r fig14_volcano_or, fig.width=6, fig.height=12}
...
ft_res_pq <- map(ft_res, ~ .x@result %>% filter(Count > 4) %>%
...
```{r further, echo=FALSE, eval=FALSE}
# Ссылки для даотнешего анализа/ изучения
https://www.bioconductor.org/packages/devel/bioc/vignettes/CEMiTool/inst/doc/CEMiTool.html
https://academic.oup.com/bioinformatics/article/37/16/2382/6149124?login=false
https://github.com/Drinchai/BloodGen3Module/
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8388021/
https://www.nature.com/articles/ni.2789?report=reader#Sec24 #with downloadable table with BTM annotations and Python script
https://www.nature.com/articles/s41590-022-01328-6
https://www.nature.com/articles/s41590-022-01361-5

...
# lmer_p_int <- readRDS("OlgaMironenko/res/lmer_p_int.rds")
# lmer_p_intrank <- readRDS("OlgaMironenko/res/lmer_p_intrank.rds")
# lmer_ame_int <- readRDS("OlgaMironenko/res/lmer_ame_int.rds")
# lmer_ame_noint <- readRDS("OlgaMironenko/res/lmer_ame_moint.rds")
# lmer_ame_intrank <- readRDS("OlgaMironenko/res/lmer_ame_intrank.rds")
# lmer_ame_nointrank <- readRDS("OlgaMironenko/res/lmer_ame_nointrank.rds")
lmer_p_int <- readRDS(file.path("..", "OlgaMironenko", "res", "lmer_p_int.rds"))
lmer_p_intrank <- readRDS(file.path("..", "OlgaMironenko", "res", "lmer_p_intrank.rds"))
lmer_ame_int <- readRDS(file.path("..", "OlgaMironenko", "res", "lmer_ame_int.rds"))
lmer_ame_noint <- readRDS(file.path("..", "OlgaMironenko", "res", "lmer_ame_noint.rds"))
lmer_ame_intrank <- readRDS(file.path("..", "OlgaMironenko", "res", "lmer_ame_intrank.rds"))
lmer_ame_nointrank <- readRDS(file.path("..", "OlgaMironenko", "res", "lmer_ame_nointrank.rds"))
```
...
Всего получается `r length(genes_int_both5_init)` гена в модели с исходными значениями для экспрессии и `r length(genes_int_both5_rank)` генов в модели с рангами для экспрессии, для которых на 5%-ном уровне значимости статистически значим и основной эффект для ответа, и эффект пересечения ответа со временем, т.е. испытуемые с разным уровнем ответа значимо отличались не только по среднему значению экспрессии данных генов на момент baseline, но и по его динамике. При этом эти списки пересекаются по `r length(intersect(genes_int_both5_init, genes_int_both5_rank))` генам: `r paste(intersect(genes_int_both5_init, genes_int_both5_rank), collapse = ", ")`.

Ещё для `r length(genes_int_b25_init)` (`r length(genes_int_b25_rank)`) генов в соответствующих моделях был значим только основной эффект, т.е. испытуемые с разным ответом на вакцинацию изначально различались по экспрессии этих генов, но её динамика была схожей в этих группах.

Ещё для `r length(genes_int_b35_init)` (`r length(genes_int_b35_rank)`) генов в соответствующих моделях был значим только эффект пересечения, т.е. испытуемые с разным ответом на вакцинацию изначально не различались по экспрессии этих генов, но её динамика была в этих группах разной.
